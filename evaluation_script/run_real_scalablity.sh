...
copy_files_from_vm() {
    ...
    copy_file_from_vm_to_host $vm2_ip $vm2_user $vm2_pass "$vm2_run_file" "$VM2_SPECIAL_FILE"
}
...
shutdown_vms
# Setting taskset for ksm 
pid=`ps aux | grep 'allocate_at_once'  | grep -v grep | awk '{print $2}'`
kill -9 $pid
