...
static inline unsigned long rand_num(unsigned long max)
{
...
void filldata( int h )
{
    addr[h] = (int*)mmap(0, size  , PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    madvise(addr , size , MADV_MERGEABLE);
    uint64_t i=0;
    while(i < size/sizeof(int)) {
       // *(addr+i) =  rand_num(10000000);
        *(addr[h]+i) =  h + (i * multiplier);
        i++;
    }
...
int main(int argc , char **argv) {
    ...
    num_of_pages = 2620000; //10GB
    //num_of_pages = 26200000; //100GB
